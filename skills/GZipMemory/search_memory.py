...
from search_core import MemoryArchiver, _search_recent_logs, _format_search_results
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python3 search_memory.py \"搜索关键词\" [天数]")
        print("示例: python3 search_memory.py \"小卡\" 60")
        sys.exit(1)
    ...
    if results["total"] == 0:
        print(f"🔍 搜索「{query}」在 {days} 天内没有找到结果")
    else:
        print(_format_search_results(
            results["recent_logs"],
            results["archived_logs"],
            memory_md_content,
            query,
            days,
        ))
